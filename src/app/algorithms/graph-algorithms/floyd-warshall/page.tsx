import React from 'react';
import type { Metadata } from 'next';
import { FloydWarshallView } from '@/features/algorithms/floyd-warshall';
import { createAlgorithmMetadata, getAlgorithmSchema } from '@/lib/seo';
import { JsonLd } from '@/components/common/json-ld';

export const metadata: Metadata = createAlgorithmMetadata({
  title: 'Floyd-Warshall Algoritması',
  description: 'Floyd-Warshall algoritması, tüm çiftler en kısa yol (APSP), matris tabanlı dinamik programlama ve interaktif görselleştirici.',
  path: '/algorithms/graph-algorithms/floyd-warshall',
  category: 'Graf Algoritmaları',
  difficulty: 'Orta',
});

export default function FloydWarshallPage() {
  const schema = getAlgorithmSchema({
    name: 'Floyd-Warshall Algoritması',
    description: 'Floyd-Warshall algoritması, tüm çiftler en kısa yol (APSP), matris tabanlı dinamik programlama ve interaktif görselleştirici.',
    category: 'Graf Algoritmaları',
    categoryHref: '/algorithms/graph-algorithms',
    path: '/algorithms/graph-algorithms/floyd-warshall',
    difficulty: 'Orta',
  });

  return (
    <div className="container py-8 max-w-7xl mx-auto">
      <JsonLd data={schema} />
      <FloydWarshallView />
    </div>
  );
}
